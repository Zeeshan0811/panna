                        ...
                        <?php if(setActive($key) && hasPermission($key, VIEW)): ?>
                            ...
                            <li class="<?php if(!empty($value[$key])) echo "has_sub" ?>">
                                <a href="<?php if(empty($value[$key])) echo site_url($value['link']); else echo "javascript:void(0)"; ?>" class="waves-effect active"><i class="md  md-view-module"></i><span><?php echo $value['name']; ?></span>
                                    ...
                                    <span class="pull-right"><i class="md md-remove"></i></span>
                                    <?php endif;?>
                                </a>
                                <?php if(!empty($value[$key])): ?>
                                    <ul class="list-unstyled" style="display: block;">
                                        <?php foreach($value[$key] as $sub_key=>$subvalue): ?>
                                            <?php if (hasPermission($subvalue['short_code'], VIEW)) : ?>
                                                <li class="<?php echo set_Submenu($subvalue['link']); ?> <?php if(!empty($subvalue[$sub_key])) echo "has_sub" ?> menu_li">
                                                    <a href="<?php if(empty($subvalue[$sub_key])) echo site_url($subvalue['link']); else echo "javascript:void(0)"; ?>"><i class="fa <?php if($subvalue['sub_icon']!='') echo $subvalue['sub_icon']; else echo "md md-trending-neutral"; ?>"></i><?php echo $subvalue['name']; ?>
